...
def main() -> None:
    ...
    average = grade_analyzer.average_class_score_per_semester()
    print(
    ...
    lowest_average = grade_analyzer.class_with_lowest_average_score()
    print("\nClass(es) with lowest average scores:")
    for subject, avg in lowest_average.items():
        print(f"{subject}: {avg}")

    grade_analyzer.to_excel(average, EXCEL_FILE)

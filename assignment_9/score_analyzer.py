...
class ScoreAnalyzer:
    ...
    def clean_data(self) -> None:
        ...
        self.df[numeric.columns] = numeric.where(numeric >= 0, np.nan)
        self.df.dropna(how="all", inplace=True)

    def failed_one_or_more_class(self) -> DataFrame:
        return self.df.loc[(self.df.iloc[:, 2:] < 50).any(axis=1), ["Student"]]

    def average_class_score_per_semester(self) -> DataFrame:
        return self.df.groupby("Semester").mean(numeric_only=True).round(2)

    def students_with_highest_average_scores(self) -> DataFrame:
    ...
    def class_with_lowest_average_score(self) -> DataFrame:
    ...
    def overall_average(self) -> DataFrame:
        temp = self.df.groupby("Semester").mean(numeric_only=True).round(2)
